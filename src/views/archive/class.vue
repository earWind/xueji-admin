...
<script setup lang="ts">
  ...
  async function queryList() {
    const { code, data } = await classListPage(unref(query));
  ...
  async function editOk() {
    ...
    try {
      const api = param.id ? classUpdate : classAdd;
  ...
  async function handleDelete(row: FormItem) {
    ElMessageBox.confirm('是否删除?', '警告', {
      ...
      .then(async () => {
        try {
          const { code, message } = await classDelete(row.id);
